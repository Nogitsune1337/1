...
// global humanization flag (reads from config at runtime)
extern bool g_humanization_enabled;
...
public:
    // Funkcja obliczająca punkt na krzywej Beziera (Quadratic Bezier)
    // t to czas od 0.0 do 1.0
    static Point GetPointOnCurve(Point p0, Point p1, Point p2, float t) {
        float invT = 1.0f - t;
        return {
    ...
    static void MoveMouseSmooth(int targetX, int targetY, float speed) {
        Point start = { 0, 0 }; // Relatywny start (0,0 bo używamy mouse_event)
        Point end = { (float)targetX, (float)targetY };

        // Tworzymy "Punkt Kontrolny" dla krzywej, aby nie była linią prostą
        // Dodajemy losowe przesunięcie, aby każdy ruch był unikalny
        Point control = {
            (start.x + end.x) / 2 + (rand() % 20 - 10),
            (start.y + end.y) / 2 + (rand() % 20 - 10)
        };

        int steps = 10 + (rand() % 5); // Liczba kroków (płynność)
        Point lastPoint = start;

        for (int i = 1; i <= steps; i++) {
            float t = (float)i / steps;
            Point currentPoint = GetPointOnCurve(start, control, end, t);

            // Obliczamy dystans, o jaki musimy się przesunąć w tym kroku
            int moveX = (int)(currentPoint.x - lastPoint.x);
            int moveY = (int)(currentPoint.y - lastPoint.y);

            // Wysyłamy ruch do systemu
            mouse_event(MOUSEEVENTF_MOVE, moveX, moveY, 0, 0);

            lastPoint = currentPoint;

            // Małe opóźnienie, aby ruch trwał w czasie (humanizacja)
            int sleepTime = (int)(1000 / (steps * speed));
    ...
    prev_y(0.0)
{
    ...
    // Ensure humanization flag is read
    humanization_enabled = g_humanization_enabled;
    // Seed rand for slight randomness in humanization
...
void MouseThread::moveWorkerLoop()
{
    while (!workerStop)
    {
        ...
        while (!moveQueue.empty())
        {
            ...
            {
                // Use bezier/humanized movement when enabled
                moveMouseBezier(m.dx, m.dy);
            }
            else
            {
                // Normal direct movement (wind behavior is handled by callers)
...
void MouseThread::moveMouseBezier(int dx, int dy)
{
    // Simple quadratic bezier relative movement implementation
    // start at (0,0), end at (dx,dy)
    Point p0{ 0.0f, 0.0f };
    Point p2{ static_cast<float>(dx), static_cast<float>(dy) };

    // control point biased by some randomness for humanization
    float randOffX = (static_cast<float>(rand()) / RAND_MAX - 0.5f) * 20.0f;
    float randOffY = (static_cast<float>(rand()) / RAND_MAX - 0.5f) * 20.0f;
    Point p1{ (p0.x + p2.x) * 0.5f + randOffX, (p0.y + p2.y) * 0.5f + randOffY };

    int steps = std::max(4, static_cast<int>(std::hypot((double)dx, (double)dy) / 8));
    Point last = p0;
    for (int i = 1; i <= steps; ++i)
    {
        float t = static_cast<float>(i) / steps;
        Point cur = BezierMovement::GetPointOnCurve(p0, p1, p2, t);
        int moveX = static_cast<int>(std::round(cur.x - last.x));
        int moveY = static_cast<int>(std::round(cur.y - last.y));
        if (moveX != 0 || moveY != 0)
        {
            sendMovementToDriver(moveX, moveY);
        }
        last = cur;
        // small randomized sleep to simulate non-uniform human motion
        int sleepUs = 100 + (rand() % 300);
